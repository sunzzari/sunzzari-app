...
struct StoriesView: View {
    ...
    /// button). Positioning handled by chromeLayer. When the current user has
    /// at least one active story today, the self-avatar badge sits directly
    /// above the plus, same size, centered. Two separate Buttons -- never
    /// nested -- so each owns its own hit region.
    ...
    /// Self-avatar badge, statically positioned directly above the plus FAB
    /// and matched to the plus button's size (title2 glyph + padding 18) so
    /// the two stack as identical circles. Tap opens a full-screen self reel.
